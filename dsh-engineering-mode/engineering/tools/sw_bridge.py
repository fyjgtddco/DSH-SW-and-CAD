...
def cmd_run(sw, script_path, extra_args):
    ...
    try:
        ...
        try:
            ...
            pass
    # 防御性检查：proc.stdout 可能为 None（编码崩溃时）
...
def cmd_dwg(sw, part_path, output_path=None):
    ...
    导出策略（按优先级）：
    1. SaveAs3 直接另存为 .dwg 扩展名（已验证可用，兼容性最好）
    2. ExportFile 方法（需要 SW 安装 DWG/DXF 翻译器插件）
    ...
    if not output_path:
        ...
        name_no_ext = os.path.splitext(part_name)[0]
        ascii_name = re.sub(r'[^\w\-]', '_', name_no_ext)
        output_path = os.path.join(os.path.dirname(part_path), ascii_name + ".dwg")
    ...
    try:
        ...
        # 方法1：SaveAs3 直接另存为 .dwg（兼容性最好，已验证可用）
        try:
            rc = doc.SaveAs3(output_path, 0, 2)
            ...
            result["saveas3_error"] = str(e1)

        # 方法2：如果 SaveAs3 失败，尝试 ExportFile（需要 DWG 翻译器插件）
        if not result.get("ok"):
            try:
                ext = doc.Extension
                export_errors = win32com.client.VARIANT(pythoncom.VT_BYREF | pythoncom.VT_I4, 0)
                export_warnings = win32com.client.VARIANT(pythoncom.VT_BYREF | pythoncom.VT_I4, 0)
                # FileFormat: 0=DWG, 1=DXF
                success = ext.ExportFile(output_path, 0, None, export_errors, export_warnings)
                if success:
                    result["ok"] = True
                    result["method"] = "ExportFile"
                    result["exists"] = os.path.exists(output_path)
                else:
                    result["exportfile_error"] = "ExportFile returned False"
            except Exception as e2:
                result["exportfile_error"] = str(e2)

        # 最终检查
        if not result.get("ok"):
            result["ok"] = os.path.exists(output_path)
            result["exists"] = result["ok"]
            if not result["ok"]:
                result["error"] = "DWG export failed (SaveAs3 and ExportFile both failed)"

    except Exception as e:
        result["ok"] = False
        result["error"] = f"DWG export failed: {e}"
